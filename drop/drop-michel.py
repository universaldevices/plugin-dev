...
dropControllerNode:udi_interface = None
dropCtrlAddress='dropctrl'
dropCtrlName='DropController'
...
class Controller(udi_interface.Node):
    ...
    def parameter_handler(self, params):
        ...
        if self.Parameters["devfile"] is not None:
            ...
            LOGGER.debug("devlist not necssary")
            return False
    ...
    def _on_message(self, mqttc, userdata, message):
        ...
        try:
            if topic_list[1] == "discovery":
                ...
                if devId == "255":
                    self._processHubData(hubId, payload)
                else:
                    self._processDeviceData(hubId, devId, payload)
                    isHub = True
