...
class CurrentValue:
    ...
    def getValue(self) -> Value:
        if self.element == None:
            return None
        try:
            return Value(self.element.find('value'))
        except:
            return None
    ...
    def getFloatValue(self) -> float :
        if self.element == None:
            return None
        try:
            return self.getValue().getFloatValue() 
        except:
            return None
...
class EiEventSignal:
    ...
    def isMode(self)->bool:
        if self.getName() == None or self.getName().lower() != "simple":
            return False
        if self.getType() == None:
            return False
        return self.getType().lower() == "level"
        
    def isPrice(self)->bool:
        if self.getName() == None:
            return False
        matches = re.findall("ELECTRICITY", self.getName(), re.IGNORECASE)
        if matches == None:
            return False
        if self.getType() == None:
            return False
        return self.getType().lower() == "price"

    def getValue():
        if self.getCurrentValue() == None:
            return False, 0
        return True, self.getCurrentValue()
