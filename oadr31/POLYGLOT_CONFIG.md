...
10. `Price Scale`
    A value to which the actual price is multiplied
...
1. **Notify Price Changes**
Notifications when prices change.  Possible options:

* `None` - No notifications at all
* `Moderate & Above` - Notification of prices only if current price is above *moderate* price (includes *high*)
* `High` - Notification of prices only if current price is *high*
* `All` - Notification of any price changes

2. **Notify DR Events**
Notifications when there are DR events. Possible options:
* `Yes` 
* `No`  

3. **Notify Device States**
Includes Device State and Opt-in/out states in the notifications. Possible options:
* `Yes` 
* `No`  
